...
import { ArrowUpRight, ArrowClockwise, Asterisk, SquaresFour, Browsers, AppWindow, Wrench, GameController, Cube, DotsThree, Info, GithubLogo, BookOpen, PaperPlaneTilt, MagnifyingGlass, X, List, CheckCircle, WarningCircle, CaretDown, ArrowRight, ImageBroken, Code, Play } from '@phosphor-icons/react';

const REPO = 'https://github.com/xianyu110/awesome-gpt-6-astra';
const GUIDE_URL = 'https://xianyu110.github.io/GPT6/';
const GUIDE_REPO = 'https://github.com/xianyu110/GPT6';
...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${menuOpen?'is-open':''}`} aria-label={language==='zh'?'主导航':'Main navigation'}>
      ...
      <div className="sidebar-secondary"><button className="nav-item" onClick={()=>{setMenuOpen(false);aboutRef.current.showModal();}}><Info size={21} weight="light"/><span>{t.about}</span></button><External className="nav-item" href={GUIDE_URL}><BookOpen size={21} weight="light"/><span>{language==='zh'?'GPT-6 指南':'GPT-6 guide'}</span><ArrowUpRight size={14}/></External><External className="nav-item" href={REPO}><GithubLogo size={21} weight="light"/><span>GitHub</span><ArrowUpRight size={14}/></External></div>
